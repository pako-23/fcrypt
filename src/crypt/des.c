...
static void permute(const uint8_t *in, const int *table, uint8_t *out,
                    int len) {
    int i, p;
...
static void do_des_encrypt(const uint8_t *plain, const uint8_t keys[16][6], uint8_t *out)
{
    ...
    for (round = 0; round < 16; ++round) {
        ...
        sblock[0] |= sbox[1][(eblock[ 0 ] & 0x03) << 4 | (eblock[1] & 0xf0) >> 4];

        sblock[1] = sbox[2][(eblock[1] & 0x0f) << 2 | (eblock[2] & 0xc0) >> 6] << 4;
        sblock[1] |= sbox[3][eblock[2] & 0x3f];

        sblock[2] = sbox[4][(eblock[3] & 0xfc) >> 2 ] << 4;
        sblock[2] |= sbox[5][(eblock[3] & 0x03) << 4 | (eblock[4] & 0xf0) >> 4];

        sblock[3] = sbox[6][(eblock[4] & 0x0f) << 2 | (eblock[5] & 0xc0) >> 6] << 4;
