...
static void permute(const uint8_t *in, const int *table, uint8_t *out, int len)
{
	int i, p;
...
static void left_rotate(uint8_t *key)
{
	int llsb, rlsb;
...
static void key_schedule(const uint8_t *key, uint8_t out[16][6])
{
	int round, cleft, cright;
	uint8_t pkey[7];

	permute(key, pc1_table, pkey, 7);
	...
	for (round = 0; round < 16; ++round) {
		...
		permute(pkey, pc2_table, out[round], 6);
	}
}

static void do_des_encrypt(const uint8_t *plain, const uint8_t keys[16][6],
			   uint8_t *out)
{
	uint8_t block[8], eblock[6], sblock[4], pblock[4], rblock[8];
	int round;

	permute(plain, ip_table, block, 8);

	for (round = 0; round < 16; ++round) {
		permute(block + 4, expansion_table, eblock, 6);

		for (int i = 0; i < 6; ++i)
			eblock[i] ^= keys[round][i];
		...
		permute(sblock, p_table, pblock, 4);

		memcpy(rblock, block + 4, 4);
		for (int i = 0; i < 4; ++i)
			rblock[4 + i] = pblock[i] ^ block[i];
		memcpy(block, rblock, 8);
	}

	memcpy(rblock, block + 4, 4);
	memcpy(rblock + 4, block, 4);

	permute(rblock, fp_table, out, 8);
...
void cr_des_decrypt(const uint8_t *ctext, const uint8_t *key, uint8_t *out)
{
	...
	for (int i = 0; i < 8; ++i) {
		...
		memcpy(keys[15 - i], buf, 6);
	}

	do_des_encrypt(ctext, keys, out);
}

void cr_tdea_encrypt(const uint8_t *plain, const uint8_t *key, uint8_t *out)
{
	uint8_t ebuf[8], dbuf[8];

	cr_des_encrypt(plain, key, ebuf);
	cr_des_decrypt(ebuf, key + 8, dbuf);
	cr_des_encrypt(dbuf, key + 16, out);
}

void cr_tdea_decrypt(const uint8_t *ctext, const uint8_t *key, uint8_t *out)
{
	uint8_t dbuf[8], ebuf[8];

	cr_des_decrypt(ctext, key + 16, dbuf);
	cr_des_encrypt(dbuf, key + 8, ebuf);
	cr_des_decrypt(ebuf, key, out);
}
